...
interface OrderItem {
  id: string; name: string; qty: number; price: number; image: any;
}
interface Order {
  orderId: string; date: string; status: OrderStatus;
  items: OrderItem[]; deliveryAddress: string;
}

// ─── Mock Data ────────────────────────────────────────────────────────────────
const orders: Order[] = [
  {
    orderId: 'FRK-10245', date: '25 Feb 2026, 12:45 PM', status: 'Delivered',
  ...
  {
    orderId: 'FRK-10201', date: '22 Feb 2026, 09:10 AM', status: 'Cancelled',
    deliveryAddress: '123 MG Road, Bengaluru',
    items: [
  ...
  {
    orderId: 'FRK-10198', date: '20 Feb 2026, 03:30 PM', status: 'Processing',
    deliveryAddress: '123 MG Road, Bengaluru',
    items: [
  ...
  {
    orderId: 'FRK-10155', date: '15 Feb 2026, 11:00 AM', status: 'Out for Delivery',
    deliveryAddress: '123 MG Road, Bengaluru',
    items: [
...
// ─── Status Config ────────────────────────────────────────────────────────────
const statusConfig: Record<OrderStatus, { color: string; bg: string; icon: string; activeStep: number }> = {
  'Delivered':        { color: '#22a55b', bg: '#e6f9ee', icon: 'checkmark-circle', activeStep: 4 },
  'Cancelled':        { color: '#e53935', bg: '#fdecea', icon: 'close-circle',     activeStep: 0 },
  'Processing':       { color: '#ff7a00', bg: '#fff4eb', icon: 'time',             activeStep: 2 },
  'Out for Delivery': { color: '#1565C0', bg: '#e3f0ff', icon: 'bicycle',          activeStep: 3 },
};

const STEPS = ['Placed', 'Processing', 'Pickup', 'Delivered'];

// ─── Stepper Component ────────────────────────────────────────────────────────
// Structure: one row for dots+lines, one row for labels underneath
const OrderStepper = ({ status }: { status: OrderStatus }) => {
  ...
  return (
    <View style={{ paddingVertical: 4 }}>
      ...
      {/* ── ROW 1: dot — line — dot — line — dot — line — dot ── */}
      <View style={{ flexDirection: 'row', alignItems: 'center' }}>
        {STEPS.map((step, idx) => {
          const isActive   = !isCancelled && idx < cfg.activeStep;
          const lineActive = !isCancelled && idx < cfg.activeStep - 1;
          ...
          return (
            <React.Fragment key={step}>
              ...
              <View style={[
                ...
                {isCancelled && idx === 0 && (
                  <Icon name="close" size={8} color="#e53935" />
                )}
              </View>

              {/* LINE between dots — flex:1 stretches to fill space */}
              {!isLast && (
                <View style={[
                  stepperStyles.line,
                  { backgroundColor: lineActive ? cfg.color : '#ddd' },
                ]} />
              )}
            </React.Fragment>
          );
      ...
      {/* ── ROW 2: labels underneath each dot ── */}
      <View style={{ flexDirection: 'row', marginTop: 6 }}>
        {STEPS.map((step, idx) => {
          ...
          return (
            <React.Fragment key={step}>
              {/* Label sits under the dot — fixed width matches dot width */}
              <Text
                style={[
                  stepperStyles.label,
                  isActive && { color: cfg.color, fontWeight: '700' },
                  isCancelled && idx === 0 && { color: '#e53935', fontWeight: '700' },
                ]}
                numberOfLines={1}
              >
                {step}
              ...
              {/* Spacer matches line width so labels stay aligned with dots */}
  ...
  return (
    <View style={styles.card}>
      ...
      <View style={styles.cardFooter}>
        <TouchableOpacity style={styles.viewBtn} onPress={() => setExpanded(!expanded)} activeOpacity={0.8}>
...
const MyOrdersScreen = () => {
  ...
  return (
    <View style={styles.container}>
      ...
      <Text style={styles.countText}>{filtered.length} order{filtered.length !== 1 ? 's' : ''} found</Text>

      {filtered.length === 0 ? (
        <View style={styles.emptyWrap}>
          <Icon name="bag-outline" size={64} color="#ddd" />
          <Text style={styles.emptyTitle}>No orders here</Text>
          <Text style={styles.emptySubtitle}>Your {activeTab.toLowerCase()} orders will appear here</Text>
        </View>
      ) : (
        <FlatList
          data={filtered}
...
const stepperStyles = StyleSheet.create({
  ...
  line: {
    flex: 1,        // stretches to fill remaining space between dots
    height: 2,
  },
  label: {
    fontSize: 9,
...
const styles = StyleSheet.create({
  ...
  tab: {
    ...
    borderRadius: 20, borderWidth: 1.5, borderColor: '#ff7a00', backgroundColor: '#fff',
  },
  ...
  previewRow: { flexDirection: 'row', alignItems: 'center', marginBottom: 4, gap: 6 },
  previewImgWrap: {
    width: 50, height: 50, borderRadius: 12,
    backgroundColor: '#fdf3ea', justifyContent: 'center', alignItems: 'center', overflow: 'hidden',
  },
  previewImg:      { width: 40, height: 40, resizeMode: 'contain' },
  moreItemsBubble: { width: 50, height: 50, borderRadius: 12, backgroundColor: '#ff7a00', justifyContent: 'center', alignItems: 'center' },
  ...
  itemImgWrap: { width: 54, height: 54, borderRadius: 12, backgroundColor: '#fdf3ea', justifyContent: 'center', alignItems: 'center' },
  ...
  totalRow:   { flexDirection: 'row', justifyContent: 'space-between', paddingTop: 12, marginTop: 6, borderTopWidth: 1.5, borderTopColor: '#f0f0f0' },
  totalLabel: { fontSize: 14, fontWeight: '700', color: '#555' },
  totalValue: { fontSize: 16, fontWeight: '800', color: '#ff7a00' },

  addressRow: { flexDirection: 'row', alignItems: 'center', marginTop: 10, gap: 6, backgroundColor: '#fff9f5', padding: 10, borderRadius: 10 },
  addressText: { fontSize: 12, color: '#666', flex: 1 },

  cardFooter: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginTop: 14, gap: 10 },
  viewBtn:    { flexDirection: 'row', alignItems: 'center', gap: 4, paddingVertical: 8, paddingHorizontal: 14, borderRadius: 10, borderWidth: 1.5, borderColor: '#ff7a00' },
  viewBtnText:   { fontSize: 13, fontWeight: '600', color: '#ff7a00' },
  actionBtnText: { fontSize: 13, fontWeight: '600', color: '#fff' },
  reorderBtn: { flexDirection: 'row', alignItems: 'center', gap: 6, paddingVertical: 8, paddingHorizontal: 16, borderRadius: 10, backgroundColor: '#ff7a00' },
  trackBtn:   { flexDirection: 'row', alignItems: 'center', gap: 6, paddingVertical: 8, paddingHorizontal: 16, borderRadius: 10, backgroundColor: '#1565C0' },
